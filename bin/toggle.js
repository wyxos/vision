...
if (dependencies['@wyxos/vision']) {
  ...
  console.error('Error: @wyxos/vision is not found in dependencies or devDependencies.');
  process.exit(1);
}
...
if (currentVersion.startsWith('file:')) {
  ...
  console.log('Switching from local development version to latest published version...');

  try {
    // Get the latest version from npm
    const latestVersion = execSync('npm show @wyxos/vision version', { encoding: 'utf8' }).trim();
    ...
    console.log(`Successfully switched to latest published version (${latestVersion}).`);
  ...
  console.log('Switching from published version to local development version...');
