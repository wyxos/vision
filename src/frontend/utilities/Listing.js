...
export default class Listing {
  ...
  async destroy(path, { props, data, config, method = 'delete' }) {
    let response
    ...
    const missingItems = refresh.query.items.filter(
  ...
  async update(path, { props, data, config, method = 'patch' }) {
    let response
  ...
  async processRowAndRefreshList({ path, props, payload, state, method } = {}) {
    ...
    const { data } = await axios[method](path || this.baseUrl, payload).catch(
      (error) => {
        rowState.failed()
