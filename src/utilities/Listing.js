...
export default class Listing {
    ...
    refreshUrl() {
        ...
        const url = base + '?' + qs.stringify(filteredParams, {arrayFormat: 'bracket'})
    ...
    async delete(options) {
        return this.processRowAndRefreshList({...options, method: 'delete', state: 'delete'})
    }

    async restore(options) {
        return this.processRowAndRefreshList({...options, method: 'patch', state: 'restore'})
    }
    ...
    async processRowAndRefreshList({path, props, payload, state, method} = {}) {
        ...
        const {data} = await this.api
            [method](path || this.baseUrl, payload)
            .catch((error) => {
                rowState.failed()
